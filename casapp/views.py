...
def index(request, auth_token=None):
    ...
    logger.debug(" index: (a) request.COOKIES: %s" % (request.COOKIES))
    ...
    logger.debug(" index: (b) uuid_session: %s" % (uuid_session))
...
def authenticate(request):
    ...
    if "session_key" in request.REQUEST:
        session_key = request.REQUEST["session_key"]
        logger.debug(" authenticate: session_key=%s" % (session_key))
        matching_sessions = filter(lambda s: s["session_key"] == session_key, SESSIONS["current"])
    ...
    logger.debug(" authenticate: response=%s" % (response))
